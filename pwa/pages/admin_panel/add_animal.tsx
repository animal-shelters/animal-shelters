...
function AddAnimal() {
  ...
  async function handleSubmit(data: any) {
    setIsBusy(true);
    axiosInstance
      .post("animals", data, { headers: { Authorization: `Bearer ${token}` } })
      .then((response) => {
        ...
        pictures.map((picture, index) => {
          urlToFile(picture.dataURL, index.toString()).then((file) => {
            if (index == 0 && pictures.length == 1) {
              axiosInstance
                .post(
                  ...
                  {
                    file: file,
                    domain: `animal/${response.data.id}`,
                    isMain: true,
                  },
                  {
                    headers: {
              ...
              axiosInstance
                .post(
                  "media_objects",
                  { file: file, domain: `animal/${response.data.id}` },
                  {
                    headers: {
                      Authorization: `Bearer ${token}`,
                      "Content-Type": "multipart/form-data",
          ...
          >
            {({
              ...
              <div className="upload__image-wrapper">
                ...
                &nbsp;
                <SecondaryButton onClick={onImageRemoveAll}>
                  Usuń wszystkie zdjęcia
                </SecondaryButton>
                {imageList.map((image, index) => (
                  <div
                    key={index}
                    className="image-item flex justify-center content-center gap-2 mt-2"
                  >
                    <img src={image.dataURL} alt="" width="100" />
                    <div className="image-item__btn-wrapper flex items-center">
                      <PrimaryButton onClick={() => onImageUpdate(index)}>
                        Aktualizuj
                      </PrimaryButton>
                      <SecondaryButton onClick={() => onImageRemove(index)}>
                        Usuń
                      </SecondaryButton>
                    </div>
                  </div>
          ...
          <Form className="w-full mt-8">
              ...
              <div className="md:w-1/3">
                ...
                >
                  ...
                  <span className="text-sm">
                    (pierwszy dzień miesiąca jeżeli data jest estymowana)
                  </span>
                </label>
              </div>
              <div className="md:w-2/3">
            ...
            <div className="flex items-center py-2 flex-row-reverse justify-items-end">
              <PrimaryButton
                busy={isBusy}
                type="submit"
                className="bg-primary dark:bg-primaryDark dark:text-white"
              >
                Zapisz
              </PrimaryButton>
              {isBusy && (
                <div
                  className="spinner-border animate-spin inline-block w-8 h-8 border-4 rounded-full ml-4"
                  role="status"
                >
